...
<script src="${webapp_url!}js/editors/tinymce/tinymce.min.js"></script>
<script>
tinyMCE.init({
...
<#macro toastr infos warnings errors msg='' msgType='info'>
...
<#macro formBreadCrumb >
<ol id="breadforms" class="breadcrumb">
	<li>
		<a href="jsp/admin/plugins/forms/ManageForms.jsp" title="Retour Liste des formulaire">
...
<button class="btn btn-link" name="${name}" type="submit" <#if !validate>formnovalidate</#if>>
   <i class='fa fa-<#if validate>check text-success<#else>remove text-danger</#if>'></i> #i18n{${label}}
</button>
</#macro>

<#macro formsButton okAction='' viewAction=''  >
<div class="form-group">
	<div class="col-xs-12 col-sm-offset-3 col-sm-8 col-md-offset-3 col-md-8">
  	<button  class="btn btn-link" id="action_${okAction}" name="action_${okAction}" type="submit">
    	<i class='fa fa-check text-success'></i> #i18n{portal.admin.message.buttonValidate}
		</button>
		<#nested>
		<#if viewAction !=''>
			<button class="btn btn-link" name="view_${viewAction}" type="submit" formnovalidate >
				<i class='fa fa-close text-danger'></i> #i18n{portal.admin.message.buttonCancel}
			</button>
		</#if>
  </div>
</div>
...
<#macro formModel >
	<@row> 
		<@columns xs=12 sm=7> 
				<fieldset>
						<legend>#i18n{forms.modify_form.labelDescription}</legend>   
				...
				<fieldset>
						<legend>#i18n{forms.create_form.labelDates}</legend>       
						<@row>
								<@columns xs=12 sm=6>   
				...
				<fieldset>
				<legend>#i18n{forms.create_form.display.labelRestrictions}</legend> 
						<@row>     
								<@columns xs=12 sm=4>  
										<@formGroup labelFor='max_number_response' labelKey='#i18n{forms.createForm.labelMaxNumberResponse}' helpKey='#i18n{forms.create_form.labelMaxNumberResponseComment.help}' rows=2>
												<@input name='maxNumberResponse' id='maxNumberResponse' type='number' value=form.maxNumberResponse?string!'0' />
										</@formGroup>
								</@columns>     
								<@columns xs=12 sm=3>       
										<@formGroup labelKey='#i18n{forms.create_form.labelAuthentificationNeeded}' helpKey='#i18n{forms.create_form.labelAuthentificationNeeded.help}' rows=2>
												<@checkBox labelKey='#i18n{forms.create_form.labelAuthentificationNeeded}' labelFor='authentificationNeeded' name='authentificationNeeded' id='authentificationNeeded' value='1' checked=(form?has_content && form.authentificationNeeded) />
										</@formGroup>
								</@columns>       
								<@columns xs=12 sm=4>           
										<@formGroup labelKey='#i18n{forms.create_form.labelLimitNumberResponse}' helpKey='#i18n{forms.create_form.labelLimitNumberResponse.help}' rows=2>
										<@checkBox labelKey='#i18n{forms.create_form.labelLimitNumberResponse}' name='oneResponseByUser' id='oneResponseByUser' value='1' checked=(form?has_content && form.oneResponseByUser) labelFor='oneResponseByUser' />
										</@formGroup>
								</@columns> 
						</@row>          
				</fieldset>
				<fieldset>
						<legend>#i18n{forms.create_form.display.label}</legend>
						<@columns xs=12 sm=6> 
		...
		<@columns xs=12 sm=5>  
		<fieldset>
				<legend>#i18n{forms.create_form.labelDisplaySummary}</legend>
						...
						<@formGroup labelKey='#i18n{forms.create_form.labelDisplaySummary}' helpKey='#i18n{forms.create_form.labelDisplaySummary.help}' rows=2>
						...
						<@formGroup labelKey='#i18n{forms.create_form.labelEndMessageDisplay}' id='labelEndMessage' helpKey='#i18n{forms.create_form.labelEndMessageDisplay.help}'  rows=2>
				...
				<#if workflow_list?has_content && workflow_list?size gt 1>
						<fieldset>
								<legend>#i18n{forms.modify_form.workflow.label}</legend>    
...
<link rel="stylesheet" type="text/css" href="js/plugins/forms/vis/vis-network.min.css" >
<script src="js/jquery/plugins/vis/vis.min.js"></script>
<script src="js/jquery/plugins/vis/jspdf.min.js"></script>
<script>
	$( function(){
		$("#renderNetwork").toggle();
		$(".state-actions").toggle();
		$(".state").nextAll().hide();
		if( $('#hierarchical').hasClass('active') ){
			$('#direction').show();
		} else {
			$('#direction').hide();
		}
	...
	/*
		var nodePosition = getStoredPositions();
		if( nodePosition != null ){
			setNodesPosition( nodePosition );
		}
	*/
	...
	*/
	...
	/* Bouton choix du type de graphique */
	$("#hierarchical").click( function(){
		$( this ).toggleClass('active');
		if( $(this).hasClass('active') ){
			$("#direction").show();
			$( this ).html('<i class="fa fa-random"></i> Pr&eacute;sentation libre');
			draw( true, 'UD' );
		} else{
			$("#direction").hide();
			$( this ).html('<i class="fa fa-sitemap"></i> Pr&eacute;sentation hi&eacute;rarchique');
			draw( false, 'UD' );
		}
	});
...
<script>
$(function() {
	$('.formsFilterDate').each( function( index )
	{
...
<th data-field="${field}" class="th-header" id="sort${id!}_${attribute!}">
	...
	<div class="btn-sort-group" role="group" aria-label="Tri">
		<a class="btn-sort asc" href="${sort_url}true#sort${id!}_${attribute!}" title="#i18n{portal.util.sort.asc}" >
			<i class="fa fa-sort-asc fa-fw"></i>
		</a>	
		<a class="btn-sort desc" href="${sort_url}false#sort${id!}_${attribute!}" title="#i18n{portal.util.sort.desc}">	
...
$( function(){ 
    var sortId=window.location.hash, sortCol=$(sortId);
		if( sortId !='' ){
			...
			/* - Colored td for selected th
				var colIndex = sortCol.index();
				$('.table tbody tr').each( function(){
					var nIndex = colIndex + 1;
					var tdCol = 'td:nth-child(' + nIndex + ')'
					$( tdCol ).toggleClass('th-active')
				});
			*/
		}
});
