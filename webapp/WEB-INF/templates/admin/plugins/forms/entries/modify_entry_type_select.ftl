...
<@row>
    ...
    <@columns md=3 lg=3 xl=3>
			<@box color='primary'>
				<#assign title>
					#i18n{forms.modifyEntry.titleQuestion} : "${form.title}"
				</#assign>
		...
		<@box color='primary'>
			<@boxHeader title='#i18n{forms.createEntry.manageField.title}' boxTools=true>
				<@tform class='form-inline pull-right' action='jsp/admin/plugins/forms/ModifyEntry.jsp?option_no_display_title=true'>
				    <input name="id_question" value="${question.id}" type="hidden" />
					<@button type='submit' name='view_createField' title='#i18n{forms.createEntry.manageField.buttonInsert}' showTitleXs=false buttonIcon='plus' size='' />
				</@tform>
			</@boxHeader>
			<@boxBody>
				<fieldset>
					...
					<@table>
						<tr> 
								...
								<th>#i18n{forms.createEntry.manageField.rowDefaultValue}</th>
						...
						<#if field.code == 'answer_choice'>
							...
							<tr>
								<td>${field.title}</td>
								<td>${field.value}</td>
								<td><#if field.defaultValue><@tag color='success'><@icon style='check' title='#i18n{forms.createEntry.manageField.rowDefaultValue}' /></@tag></#if></td>
								<td>
