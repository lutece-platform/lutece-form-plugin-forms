...
<@row>
	<@columns>
		<@box color='primary'>
			...
			<@boxBody>
				<fieldset>
					<legend class="sr-only">#i18n{forms.modifyEntry.titleComment} : '${form.title}'</legend>
                    ...
                    <input name="id_question" value="${question.id}" type="hidden" />
						...
						<@formGroup>
							...
							<@button type='button' name='cancel' buttonIcon='close' title='#i18n{forms.createEntry.buttonCancel}' showTitleXs=false size='' color='btn-secondary' params='onclick="javascript:window.history.back();"' />
						</@formGroup>
					</@tform>
				</fieldset>
			</@boxBody>
