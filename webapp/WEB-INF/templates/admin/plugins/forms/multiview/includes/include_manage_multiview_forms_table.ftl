        ...
        <tbody>
            <#if form_response_line_template_list?has_content>
                <#list form_response_line_template_list as form_response_line_template>
                    <tr data-url="${redirect_details_base_url}&id_form_response=${form_response_line_template.idFormResponse}">
...
<script id="geojson_points" type="application/json">{
	...
	"features" : [
<#list form_response_geojson_point_list as point>${point?replace("</","<\\/")}<#if point_has_next>,
</#if>
</#list>
]
}
