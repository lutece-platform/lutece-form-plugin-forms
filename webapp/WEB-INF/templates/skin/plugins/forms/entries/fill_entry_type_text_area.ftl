...
<#if !entry.onlyDisplayInBack || is_completeness_bo>
	<#macro getTextAreaContent responseContent has_error entry >
		...
		<#if has_error>
			<textarea name="${idName}" id="${idName}" cols="${field_width?number}" rows="${field_height?number}" class="alert alert-danger form-control">${responseContent}</textarea>
		<#else>
			<textarea name="${idName}" id="${idName}" cols="${field_width?number}" rows="${field_height?number}" class="${entry.CSSClass!} form-control">${responseContent}</textarea>
		</#if>
	</#macro>
